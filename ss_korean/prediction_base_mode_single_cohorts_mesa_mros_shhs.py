...
fold = 0 
path1 = '/media/exx/Expansion1/CAISR/extracted_features/sleep_staging/'
...
def predict(model_path, spec_path, sig_path, lab_path, fold):
    ...
    path = '/media/exx/Expansion1/CAISR/prepared_data/'
    ...
    if dataset [0] == 'mgh':
        ...
        path_csv = '/media/exx/Expansion1/codes/koges_caisr/csv_files_70_15_15/'
        df_self_data = pd.read_csv(path_csv + 'missing_stage.csv')
        import pdb; pdb.set_trace()
        test_line = df_self_data['0'].values.tolist()
        test_cohort_line = df_self_data['dataset'].values.tolist()

    # import pdb; pdb.set_trace()
    # input_path = path + dataset[0] + '/' 
    # files = os.listdir(input_path)
    

    csv_folder = '/media/exx/Expansion1/CAISR/caisr_output/stage/mgh_graph_2024_02_23_mgh_test/'
    already_predicted_files = os.listdir(csv_folder)

    already_predicted_files = [filename.replace('_stage.csv', '') for filename in already_predicted_files]
    test_line = [x for x in test_line if x not in already_predicted_files]
    test_cohort_line = dataset * len (test_line)

    # test_line = [x for x in files if x not in already_predicted_files]
    # test_cohort_line = dataset * len (test_line)
    # if dataset [0] == 'robert_v6':
    #         input_folder = '/media/exx/Expansion1/codes/koges_caisr/csv_files_70_15_15/'
    #         filename = 'files_few_shot_robert_fold2.csv'
    #         df_files = pd.read_csv(input_folder + filename)
    #         input_path = input_folder + dataset[0] + '/' 
    #         test_line = df_files['0'].values.tolist()
    #         test_cohort_line = dataset * len (test_line)
    ...
    for i in range(0, len(test_line)):
        ...
        try:
            ...
            image, _ = load_features_and_labels_unseen(sig_path, lab_path, dataset, the_id)
        ...
        df_matched = pd.DataFrame({'start_idx':t1,'end_idx':t2,'stage':np.nan,'prob_n3':np.nan,'prob_n2':np.nan,'prob_n1':np.nan,'prob_r':np.nan,'prob_w':np.nan})
        df_matched.iloc[0:len(df)] = df
        spec_path = '/media/exx/Expansion1/CAISR/caisr_output/stage/' + dataset + '_graph_2024_02_23_mgh_test/'
        # import pdb; pdb.set_trace()
        os.makedirs(spec_path, exist_ok=True) 
...
def main():
        ...
        parser.add_argument('--model_path', type=str, default = '/media/exx/Expansion1/codes/koges_caisr/Stage/Models/graphsleepnet_ self-testing_MGH_1_SS_Feb_22_2024_context_7_batch_5_atten_40_epoch_80/',
        help = 'signal path, default = /media/exx/Expansion1/codes/koges_caisr/Stage/Models/models/graphsleepnet_ self-testing_MGH_1_SS_Feb_22_2024_context_7_batch_5_atten_40_epoch_80/')
        
        parser.add_argument('--spec_path', type=str, default = '/media/exx/Expansion1/CAISR/caisr_output/stage/mesa_graph_2024_01_29/',
        help = 'spec path, default = /media/exx/Expansion1/CAISR/caisr_output/stage/mesa_graph_2024_01_29/')

        parser.add_argument('--sig_path', type=str, default = '/media/exx/Expansion1/CAISR/extracted_features/sleep_staging/',
        help = 'signal path, default = /media/exx/Expansion1/CAISR/extracted_features/sleep_staging/')
